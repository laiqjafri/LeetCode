...
class Solution:
    ...
    #         if not cost or not gas or len(cost) != len(gas): return -1
    ...
    #             if i == curr and sufficient_balance:
    #                 return i

    #         return -1

    def canCompleteCircuit(self, gas: List[int], cost: List[int]) -> int:
        if not cost or not gas or len(cost) != len(gas) or sum(gas) < sum(cost): return -1
